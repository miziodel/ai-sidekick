...
function prepareSystemInstruction(customInstruction) {
  const base =
    'You are AI Sidekick, a helpful browser assistant. Be concise, accurate, and use Markdown for formatting.';
...
function parseMarkdown(text) {
  ...
  // 2. Basic Sanitization (since we lack DOMPurify)
  ...
  // This regex looks for on[a-z]+ followed by = inside a tag.
  // It's not perfect but blocks low-hanging fruit like onerror=.
  // Match: <tag ... onEvent= ... >
  html = html.replace(/\son[a-z]+\s*=\s*(?:'[^']*'|"[^"]*"|[^\s>]+)/gim, ' ');

  // Remove javascript: links
  html = html.replace(
    /href\s*=\s*(?:'javascript:[^']*'|"javascript:[^"]*"|javascript:[^\s>]+)/gim,
    'href="#"',
  );
